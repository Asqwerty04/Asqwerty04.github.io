...
Жестокий век.prototype = {
	...
	,removeBookmark: function(pageNum) {
		...
		while(_g1 < _g) {
			var i1 = _g1++;
			haxe.Log.trace(RunTime.book.bookmarks[i1].pageNum,{ fileName : "Жестокий век.hx", lineNumber : 1722, className : "Жестокий век", methodName : "removeBookmark"});
...
var DoubleЖестокий век = function() {
	this.mainAdLayout = "center";
	this.mainAdDockPos = "halfpage";
	Жестокий век.call(this);
};
...
RunTime.loadState = function() {
	...
	if(bbv == true) RunTime.Жестокий век.showBottomBar(); else RunTime.Жестокий век.hideBottomBar(null,false);
}
...
RunTime.preRequestBookInfo = function() {
	orc.utils.Util.request(RunTime.urlBookinfo,function(data) {
		...
		RunTime.Жестокий век.btnAboutUs = js.Lib.document.getElementById("btnAboutUs");
		...
		RunTime.Жестокий век.topBarContent.style.zIndex = 10000;
...
RunTime.InputPwd = function() {
	...
	RunTime.Жестокий век.cvsOthers.innerHTML = core.HtmlHelper.toInputPwdHtml();
}
RunTime.InputUnlock = function() {
	RunTime.showPopupMaskLayer();
	RunTime.Жестокий век.cvsOthers.innerHTML = core.HtmlHelper.toInputUnlockPwdHtml();
}
...
RunTime.afterRequestBookInfo = function() {
	RunTime.Жестокий век.cvsOthers.innerHTML = "";
	RunTime.clearPopupContents();
...
RunTime.requestButtons = function(onSuccess) {
	orc.utils.Util.request(RunTime.urlButtons,function(data) {
		...
		RunTime.loadButtons();
		if(RunTime.Жестокий век != null) {
			RunTime.Жестокий век.loadCtxButtons();
			RunTime.Жестокий век.bookContext.render();
		}
		if(onSuccess != null) onSuccess();
	});
...
RunTime.reload = function() {
	js.Lib.window.location.href = RunTime.Жестокий век.getFullUrl();
}
...
//      RunTime.book.menuAboutUsVisible = RunTime.getMenuVisible(leftMenuNode, "about")
	...
	var menuArray = [RunTime.Жестокий век.btnContents,RunTime.Жестокий век.btnThumbs,RunTime.Жестокий век.btnSearch,RunTime.Жестокий век.btnShowTxt,RunTime.Жестокий век.btnMask,RunTime.Жестокий век.btnBookMark,RunTime.Жестокий век.btnAutoFlip,RunTime.Жестокий век.btnEmail,RunTime.Жестокий век.btnSns,RunTime.Жестокий век.btnAboutUs];
...
RunTime.setMenuVisible = function(menu,visible) {
	if(visible == true) menu.style.display = "inline"; else try {
		RunTime.Жестокий век.menuParent.removeChild(menu);
	} catch( e ) {
	}
}
...
RunTime.getInputAndJumpToPage = function() {
	RunTime.Жестокий век.stopFlip();
	var t = RunTime.Жестокий век.tbPage;
	var val = t.value;
	val = StringTools.trim(val);
	var num = RunTime.Жестокий век.currentPageNum;
	if(val != "") num = Std.parseInt(val) - 1;
	if(num < 0) num = 0; else if(num > RunTime.book.pages.length - 1) num = RunTime.book.pages.length - 1;
	RunTime.Жестокий век.tbPage.setAttribute("value",Std.string(num + 1));
	RunTime.Жестокий век.turnToPage(num);
	RunTime.Жестокий век.tbPage.blur();
}
...
RunTime.saveCurrentPageNum = function() {
	RunTime.savePageNum(Std.string(RunTime.Жестокий век.getCurrentPageNum()));
}
...
RunTime.clearPopupContents = function() {
	...
	RunTime.Жестокий век.resetNoteButton();
	RunTime.Жестокий век.resetHighlightButton();
}
...
RunTime.readLocalHighLights = function() {
	...
	if(RunTime.Жестокий век != null) {
		RunTime.Жестокий век.loadCtxHighLights();
		RunTime.Жестокий век.bookContext.render();
	}
	return highlights;
}
...
RunTime.readLocalNotes = function() {
	...
	RunTime.notes = notes;
	if(RunTime.Жестокий век != null) {
		RunTime.Жестокий век.loadCtxNotes();
		RunTime.Жестокий век.bookContext.render();
	}
	return notes;
}
		...
		case "":
			if(this.destination != null) {
				if(this.destination.indexOf("page:") == 0) {
					...
					if(RunTime.Жестокий век != null) RunTime.Жестокий век.turnToPage(num - 1); else Zoom.zoomOut(num - 1);
					...
					if(fun == "content") RunTime.Жестокий век.onContentsClick(null); else if(fun == "thumb") RunTime.Жестокий век.onThumbsClick(null); else if(fun == "showtxt") RunTime.Жестокий век.onShowTxtClick(null); else if(fun == "highlight") RunTime.Жестокий век.onButtonMaskClick(null); else if(fun == "bookmark") RunTime.Жестокий век.onButtonBookmark(null); else if(fun == "notes") RunTime.Жестокий век.onButtonNoteClick(null); else if(fun == "autoflip") RunTime.Жестокий век.onAutoFlipClick(null); else if(fun == "download") RunTime.onDownloadClick(null); else if(fun == "fliptofront") RunTime.Жестокий век.turnToFirstPage(null); else if(fun == "flipleft") RunTime.Жестокий век.turnToPrevPage(null); else if(fun == "flipright") RunTime.Жестокий век.turnToNextPage(null); else if(fun == "fliptoback") RunTime.Жестокий век.turnToLastPage(null);
				} else {
					RunTime.logClickLink(this.destination);
